...
def _rust_supported_subset(enc: _HV) -> tuple[bool, str]:
    ...
    if getattr(enc, "norm", "l2") not in (None, "l2"):
        return False, "norm not in {None, 'l2'}"
    try:
        dtype = np.dtype(getattr(enc, "dtype", np.float64))
    except (TypeError, ValueError):
        return False, "invalid dtype"
    if dtype != np.dtype(np.float64):
        return False, "dtype is not exactly supported"
...
class RustyHashingVectorizer(_HV):
    ...
    def _rust_transform(self, X):
        ...
        if any(not document.isascii() for document in docs):
        ...
        try:
            ...
            return super().transform(docs)
        rb.print_timing("hashing_vectorizer_transform", t0)

        X_out = sp.csr_matrix(
            (data, indices, indptr), shape=(n_rows, n_cols), dtype=self.dtype
        )
        return X_out

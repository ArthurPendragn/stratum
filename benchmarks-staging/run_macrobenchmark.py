...
Workers report wall time, process peak RSS, kernel RSS delta, and CSR
nbytes. By default the rust leg also enables ``debug_timing`` for one
extra transform and harvests stage timings (see ``DEBUG_TIMING.md``).
"""
...
def run_worker(
    ...
    # Pair memory with the fastest timed transform (same index as time_ms).
